...
public class SMPS_Service {
        ...
        for ( int i = 0 ; i< serverListDetails.size() ; i++) {
            ...
            if (totalCpu != null && totalMem != null) {
                ...
                if ((totalCpu < cpuMin || totalCpu < cpuMax) || (totalMem < memMin || totalMem > memMax)) {
                    emailService.sendEmail("guptaaditya9728@gmail.com", "Error", mailBody);
                }
    ...
    public void runBatchFile() throws IOException, InterruptedException {
        ...
        List<String> batchFileTotalProcessesList = readBatchFile(batchFileText);
        ...
        for (String targetProcess : findListOfSpecificProcessFromServerMasterDB()){
            ...
            for (OSProcess totalProcessesList : processList){
                ...
                //1
                /*serverCpuUtil = serverCpuUtil + totalProcessesList.getProcessCpuLoadCumulative()*100;
                serverMemUtil = serverMemUtil + ( totalProcessesList.getResidentSetSize()/(1024L*1024L) );  //mb*/

                if (totalProcessesList.getName().equals(targetProcess)){
